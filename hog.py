...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while i < num_rolls:
        roll = dice()
        if roll == 1:  # will return 1 regardless, but continue rolling for gui
            one_rolled = True
        total = total + roll
        i = i + 1
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    for iter in range(1,11):
        item_result = make_averaged(roll_dice)(iter, dice)
...
def bacon_strategy(score, opponent_score, margin=8, num_rolls=5):
    ...
    """
    "*** YOUR CODE HERE ***"
    diff = abs (opponent_score // 10 - opponent_score % 10) + 1
    if diff >= margin:
        return 0
    else:
        return num_rolls
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=5):
    ...
    """
    "*** YOUR CODE HERE ***"
    if score + take_turn(0, opponent_score, six_sided) == opponent_score / 2:
        return 0
    elif score + take_turn(0, opponent_score, six_sided) == opponent_score * 2:
        return num_rolls
    else:
        if take_turn(0, opponent_score, six_sided) >= margin:
            return 0
        else:
            return num_rolls
